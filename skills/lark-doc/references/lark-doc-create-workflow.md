...
- **表达一致**：同一对象、动作和状态全文同名；标题层级与编号采用统一体系，如下；用户提供样例时，在不违反更高优先级规则的前提下延续其有效结构、语气、术语和编号。
  - **自动编号模式（默认）**：每一个正文标题都写 `seq="auto"`，标题文本不手写任何前置序号；章用 `<h1>`、节用 `<h2>`。表格默认浅紫表头 + 浅蓝首列。详见 [`../../feishu-doc-format/SKILL.md`](../../feishu-doc-format/SKILL.md)。
  - **中文手写模式**：仅当用户明确要求公文序号时使用，在标题文本中手写 `一、→（一）→ 1.→（1）`；最忌中文层级配阿拉伯小数，绝不出现 `一、` 下接 `1.1`。
