...
用户只说「画图 / 架构图 / /画图」、没有 Skill 名、也没有对照截图时：

1. 用 AskQuestion 列出 §1（中文名 + skill 名），等用户选。
2. 选定后再 Read 该 Skill。
