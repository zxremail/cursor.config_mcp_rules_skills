...
- [如何确定目录](#如何确定目录)
- [必须先做](#必须先做)
- [索引表怎么用（存在索引时）](#索引表怎么用存在索引时)
- [与仓库真源的关系](#与仓库真源的关系)
- [维护索引（用户让你改笔记时）](#维护索引用户让你改笔记时)
- [阅读顺序](#阅读顺序)
- [1. 如何确定目录](#1-如何确定目录)
- [2. 必须先做](#2-必须先做)
- [3. 索引表怎么用（存在索引时）](#3-索引表怎么用存在索引时)
- [4. 与仓库真源的关系](#4-与仓库真源的关系)
- [5. 维护索引（用户让你改笔记时）](#5-维护索引用户让你改笔记时)
- [6. 阅读顺序](#6-阅读顺序)
...
- **同步文档内目录**：改完 `.md` 正文后，按 **`markdown-knowledge-maintain`** skill 运行 `~/.cursor/tools/md-toc/md-toc` 补充 `## 目录`（已有目录时只追加缺失章节，禁止整段重写；**默认纯 Markdown**，勿插入 HTML 锚点/回链）。
