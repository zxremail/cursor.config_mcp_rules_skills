...
description: >-
  ...
  或用户抱怨「连线乱/交叉」「图太大/空白太多」「小图底下多余图例」时应用。
---
...
涵盖：**多 subgraph 换行**、**跨域架构图少交叉**、**语义配色**、**箭头说明**、**小图按内容尺寸显示且不加图例**、**仅复杂架构图才用独立图例**。
...
| 需要图例（仅 §6.2） | 主图 `TB` + 图例 `LR` | — | 图例独立块 | §6.3 |
